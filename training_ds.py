...
test_img = '/tmp/mall_dataset/frames/seq_002000.jpg'
...
train_ds = total_ds.take(1600)
train_ds = train_ds.shuffle(buffer_size=1600)
train_ds = train_ds.repeat()
train_ds = train_ds.batch(batch_size)
train_ds = train_ds.prefetch(buffer_size = tf.data.experimental.AUTOTUNE)


validation_ds = total_ds.skip(1600)
...
model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate=0.00001),
              ...
              metrics=['mae'])
model.fit(x=train_ds, validation_data=validation_ds, steps_per_epoch=200, 
           validation_steps=1, epochs = 50, callbacks=[tensorboard_callback,
                                                       den_callback])
