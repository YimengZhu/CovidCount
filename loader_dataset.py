...
class MallDataset():
    def __init__(self, data_folder, class_num):
        image_folder = os.path.join(data_folder, 'val', '*.jpg')
        density_folder = os.path.join(data_folder, 'val_den', '*.bmp')
        ...
        #label_file = os.path.join(data_folder, 'count_random_aug.mat')
        ...
        #self.cls = (self.count - min_count)//self.class_distance 
    ...
    def dataset(self):
       ...
       #                                            "output_2":density_ds}))
        total_ds = tf.data.Dataset.zip((image_ds, density_ds))
        return total_ds
